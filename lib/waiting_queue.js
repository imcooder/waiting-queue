...
module.exports = class WaitingQueue extends EventEmitter {
   ...
   */
    ...
    add(request, opt) {
        ...
        opt = opt || {};
        opt.waitTimeout = opt.waitTimeout || 5000;
    ...
    _pushTask(request, opt) {
        ...
        let promise = new Promise((resolve, reject) => {
            ...
            taskInfo.waitTimer = setTimeout(() => {
                if (['finish', 'cancel', 'timeout', 'doing'].indexOf(taskInfo.status) !== -1) {
                    return;
                }
    ...
    _process(taskInfo) {
        ...
        let callback = taskInfo.request.callback;
        if (!callback) {
            this._error('invalid_callback');
            if (taskInfo.reject) {
                taskInfo.reject(new Error('invalid_callback'));
            }
            return;
        }
        if (taskInfo.waitTimer) {
            clearTimeout(taskInfo.waitTimer);
            taskInfo.waitTimer = null;
        }
        taskInfo.status = 'doing';
        this._runningNum ++;
        this._debug('running %d', this._runningNum);
        callback(taskInfo.request).then(data => {
            this._runningNum --;
